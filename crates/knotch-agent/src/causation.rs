...
//!
//! - `source = Source::Hook`
//! - `principal = Principal::Agent { agent_id, model, harness }` — `agent_id` comes from
//!   the hook's `session_id`, `model` comes from `KNOTCH_MODEL` env var (falls back to
//!   `"unknown"`), `harness` comes from `KNOTCH_HARNESS` (falls back to `"claude-code"`).
