...
use knotch_kernel::{Log, PhaseKind, Proposal, Scope, UnitId, WorkflowKind, event::SkipKind};
...
impl WorkflowKind for Wf {
    ...
    fn name(&self) -> std::borrow::Cow<'_, str> {
        std::borrow::Cow::Borrowed("cancel-fixture")
    }
    ...
    async fn observe<'ctx>(
        ...
        loop {
            if ctx.cancel.is_cancelled() {
                return Err(ObserverError::Cancelled { name: "slow".into(), elapsed_ms: 0 });
            }
